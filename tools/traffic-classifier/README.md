...
`sample/` is a proxymock recording: five minutes of traffic through this banking demo, 594 inbound calls and 53 sessions, one readable markdown file per call. The run finishes in a couple of seconds. Open `out/report.md`. A model-named version of the same run is checked in as [`sample-report.md`](sample-report.md).
...
1. **Identity.** Each call is attributed to whoever made it. If the capture came through Speedscale with session tracking configured, the `session` tag on each call is used as-is, since it reflects rules the tenant owner set on purpose. Otherwise: the subject in a JWT, then a username or email in the body or query string (so the anonymous start of a login stitches onto the authenticated remainder), then a hashed API key or tenant header for machine callers. Sessions are keyed on cluster plus identity, because one capture can carry more than one environment.
