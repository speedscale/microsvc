...
This repro isolates the Pega OTEL report to the plaintext OTLP/gRPC path:
...
```bash
cd kubernetes/testing/otel-grpc-repro
...
```bash
kubectl -n tracing logs deployment/tracing-agent --tail=200 | grep pega.otlp.grpc.export
```
...
If the Pega symptom reproduces, the useful evidence is the agent deployment logs plus collector logs from the same time window.
